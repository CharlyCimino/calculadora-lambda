...
var constant_or = new Primitive(
    ...
    2,
    (x,y) => x.equals(pri_alitrue) || y.equals(pri_true) ? pri_true : pri_false);
...
function addUntypedMacros() {
    ...
    a("SUCC",   succuntyped);
    a("PRED",   pred);
    a("DIV",    parse("(λn.(((λf.((λx.(f (x x))) (λx.(f (x x))))) (λc.(λn.(λm.(λf.(λx.((λd.((((λn.((n (λx.(λx.(λy.y)))) (λx.(λy.x)))) d) (((λf.(λx.x)) f) x)) (f ((((c d) m) f) x)))) (((λm.(λn.((n (λn.(λf.(λx.(((n (λg.(λh.(h (g f))))) (λu.x)) (λu.u)))))) m))) n) m)))))))) ((λn.(λf.(λx.(f ((n f) x))))) n)))"));
    a("Ω",      alias_Omega);
    a("OMEGA",  alias_Omega);
    a("Θ",      parse("((λx.(λf.(f ((x x) f)))) (λx.(λf.(f ((x x) f)))))"));
    a("THETA",  parse("((λx.(λf.(f ((x x) f)))) (λx.(λf.(f ((x x) f)))))"));
    a("ISZERO", iszero);
    a("LEQ",    parse("λm.λn.ISZERO (MINUS m n)"));
    a("EQ",     parse("(λm.(λn.((AND ((LEQ m) n)) ((LEQ n) m))))"));
    a("Y",      YCOMB);

    a("S", parse("(λxyz.((xz)(yz)))"));
    a("K", parse("(λij.i)"));
    a("I", parse("(λi.i)"));
