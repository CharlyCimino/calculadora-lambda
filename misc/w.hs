import qualified Data.Map as Map -- environment
import qualified Data.Set as Set -- substitutions
...
data Exp = EVar String -- he keeps variable as string
         | ELit Lit
         | EApp Exp Exp
         | EAbs String Exp
         | ELet String Exp Exp
...
data Lit = LInt Integer -- constants (primitives)
         | LBool Bool
         deriving (Eq, Ord)
...
data Scheme = Scheme [String] Type -- forall [variable] type

class Types a where
    ftv :: a->Set.Set String -- set of free (type) variables
    apply :: Subst -> a -> a -- apply substitution on type

instance Types Type where
    ftv (TVar n) = Set.singleton n
    ftv TInt     = Set.empty
    ftv TBool    = Set.empty
    ftv (TFun t1 t2) = Set.union (ftv t1) (ftv t2) -- union
...
instance Types a => Types [a] where
    apply s = map $ apply s
    ftv t = foldr Set.union Set.empty (map ftv t)

type Subst = Map.Map String Type -- type variable to type
...
ti (TypeEnv env) (EVar n) =  -- ok
    case Map.lookup n env of 
       Nothing     ->  throwError $ "unbound variable: " ++ n
       Just sigma  ->  do  t <- instantiate sigma
                           return (nullSubst, t)
ti _ (ELit l) = tiLit l -- ok
ti env (EAbs n e) = -- ok
    do  tv <- newTyVar "a"
...
e7 = EApp (EAbs "x" (EApp(EVar "x") (ELit (LInt 2)))) (EAbs "x" (EVar "x"))

e8 = ELet "id" (EAbs "x" (EVar "x")) (EApp (EVar "id") (EVar "id"))
